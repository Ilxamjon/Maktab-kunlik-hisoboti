...
public class MainActivity extends Activity {
    ...
    void onboarding(String idToken){work(()->api("/catalog",null),catalog->{
        ...
        Spinner districtPick=new Spinner(this);districtPick.setAdapter(new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,districtNames));page.addView(districtPick);EditText districtManual=input("Yangi tuman nomi (faqat ro‘yxatda bo‘lmasa)","");districtManual.setVisibility(View.GONE);
        ArrayList<JSONObject> selectedSchools=new ArrayList<>();ArrayList<String> schoolNames=new ArrayList<>();ArrayAdapter<String> schoolAdapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,schoolNames);Spinner schoolPick=new Spinner(this);schoolPick.setAdapter(schoolAdapter);page.addView(schoolPick);EditText schoolManual=input("Yangi maktab raqami yoki nomi","");schoolManual.setVisibility(View.GONE);
        ArrayList<String> classNames=new ArrayList<>();ArrayAdapter<String> classAdapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_dropdown_item,classNames);Spinner classPick=new Spinner(this);classPick.setAdapter(classAdapter);page.addView(classPick);EditText classManual=input("Yangi sinf, masalan 5-A","");classManual.setVisibility(View.GONE);
        ...
        districtPick.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> a,View v,int p,long id){districtManual.setVisibility(p>=districts.length()?View.VISIBLE:View.GONE);refreshSchools.run();}public void onNothingSelected(AdapterView<?> a){}});
        schoolPick.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> a,View v,int p,long id){schoolManual.setVisibility(p>=selectedSchools.size()?View.VISIBLE:View.GONE);refreshClasses.run();}public void onNothingSelected(AdapterView<?> a){}});
        classPick.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener(){public void onItemSelected(AdapterView<?> a,View v,int p,long id){classManual.setVisibility(p>=classNames.size()-1?View.VISIBLE:View.GONE);}public void onNothingSelected(AdapterView<?> a){}});refreshSchools.run();
        ...
        label("Birinchi ro‘yxatdan o‘tgan foydalanuvchi direktor o‘rinbosari bo‘ladi. Keyingi o‘rinbosar so‘rovini hozirgi o‘rinbosar tasdiqlaydi; sinf rahbarlari ham uning tasdig‘idan keyin ishlay oladi.");
    ...
    void pending(){screen("Tasdiqlash kutilmoqda");label("So‘rovingiz maktab direktor o‘rinbosariga yuborildi. Tasdiqlangach Google orqali qayta kiring.");button("Qayta kirish",this::login);}
    ...
    void settings(){work(()->api("/settings",null),r->{
        ...
        label("Telegram botni ulash");
        button("Telegram botni ulash yo‘riqnomasi",this::telegramGuide);
        EditText botToken=input("BotFather bergan Bot token (serverda bo‘lsa bo‘sh qoldiring)","");botToken.setInputType(129);
        button("BotFather’ni ochish",()->startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse("https://t.me/BotFather"))));
    ...
    void telegramGuide(){
        new AlertDialog.Builder(this).setTitle("Telegram botni ulash")
            .setMessage("1. Telegramda @BotFather botini oching.\n\n2. /newbot buyrug‘ini yuboring.\n\n3. Bot nomini, keyin username kiriting. Username bot bilan tugashi kerak.\n\n4. BotFather yuborgan tokenni to‘liq nusxalang. Tokenni hech kimga bermang.\n\n5. Ilovaga qaytib tokenni kiriting va ‘1. Telegram botni ochish’ tugmasini bosing.\n\n6. Ochilgan botda Start tugmasini bosing.\n\n7. Ilovaga qaytib ‘2. Ulanishni tekshirish’ tugmasini bosing.\n\n‘Telegram muvaffaqiyatli ulandi’ yozuvi chiqsa, PDF hisobotlar shu akkauntga keladi.")
